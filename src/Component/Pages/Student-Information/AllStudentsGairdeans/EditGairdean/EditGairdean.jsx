...
const EditGairdean = () => {
  ...
  const [gairdeanData, setGairdeanData] = useState();
  const { id } = useParams();
  // console.log(id);
  const {
    register,
  ...
  const onSubmit = async (data) => {
    const newData = {
      studentId: gairdeanData.studentId._id,
      whatsapp: data.whatsapp ? data.whatsapp : gairdeanData.whatsapp,
      bloodGroup: data.bloodGroup ? data.bloodGroup : gairdeanData.bloodGroup,
      deathYear: data.deathYear ? data.deathYear : gairdeanData.deathYear,
      email: data.email ? data.email : gairdeanData.email,
      facebook: data.facebook ? data.facebook : gairdeanData.facebook,
      geirdeanNameBn: data.geirdeanNameBn
        ? data.geirdeanNameBn
        : gairdeanData.geirdeanNameBn,
      geirdeanNameEn: data.geirdeanNameEn
        ? data.geirdeanNameEn
        : gairdeanData.geirdeanNameEn,
      mobilenumber: data.mobilenumber
        ? data.mobilenumber
        : gairdeanData.mobilenumber,
      monthlyIncome: data.monthlyIncome
        ? data.monthlyIncome
        : gairdeanData.monthlyIncome,
      nidNumber: data.nidNumber ? data.nidNumber : gairdeanData.nidNumber,
      occupation: data.occupation ? data.occupation : gairdeanData.occupation,
      workLocation: data.workLocation
        ? data.workLocation
        : gairdeanData.workLocation,
      geirdeanImage: image ? image : gairdeanData.geirdeanImage,
    };

    try {
      setIsload(true);
      const submittedData = await PatchData(
        `gairdean/update-single-gairdean-by-patch/${gairdeanData._id}`,
        newData
      );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-md">
      ...
      <h2 className="text-2xl font-semibold text-center mb-6">
        Edit Gairdean Information
      </h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {/* Father's Name in English */}
        <div>
          <label htmlFor="motherNameEn" className="block font-medium mb-1">
            Geirdean`s Name (English)
          </label>
          <input
            type="text"
            id="geirdeanNameEn"
            {...register("geirdeanNameEn", {
              required: false,
            })}
            className="w-full border rounded-lg p-2"
            placeholder="Enter mother's name in English"
          />
          {errors.geirdeanNameEn && (
            <p className="text-red-500 text-sm">
              {errors.geirdeanNameEn.message}
            </p>
          )}
        </div>
        ...
        <div>
          <label htmlFor="geirdeanNameBn" className="block font-medium mb-1">
            অবিভাবকের নাম (বাংলা)
          </label>
          <input
            type="text"
            id="geirdeanNameBn"
            {...register("geirdeanNameBn", {
              required: false,
            })}
            className="w-full border rounded-lg p-2"
            placeholder="অবিভাবকের নাম বাংলায় লিখুন"
          />
          {errors.geirdeanNameBn && (
            <p className="text-red-500 text-sm">
              {errors.geirdeanNameBn.message}
            </p>
          )}
        </div>
            ...
            <div className="w-full relative flex-col cursor-pointer h-auto min-h-[150px] rounded-md overflow-hidden border flex justify-center items-center">
              {image ? (
                ...
                <div className="w-full max-h-[100px] min-h-[150px] flex flex-col justify-center items-center h-full">
                  ...
                  <small>Upload Geirdean Image</small>
                </div>
              )}

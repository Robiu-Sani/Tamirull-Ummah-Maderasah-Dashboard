...
const EditMother = () => {
  ...
  const onSubmit = async (data) => {
    const newData = {
      studentId: motherData.studentId._id,
      whatsapp: data.whatsapp ? data.whatsapp : motherData.whatsapp,
      bloodGroup: data.bloodGroup ? data.bloodGroup : motherData.bloodGroup,
      deathYear: data.deathYear ? data.deathYear : motherData.deathYear,
      email: data.email ? data.email : motherData.email,
      facebook: data.facebook ? data.facebook : motherData.facebook,
      motherNameBn: data.motherNameBn
        ? data.motherNameBn
        : motherData.motherNameBn,
      motherNameEn: data.motherNameEn
        ? data.motherNameEn
        : motherData.motherNameEn,
      mobilenumber: data.mobilenumber
        ? data.mobilenumber
        : motherData.mobilenumber,
      monthlyIncome: data.monthlyIncome
        ? data.monthlyIncome
        : motherData.monthlyIncome,
      nidNumber: data.nidNumber ? data.nidNumber : motherData.nidNumber,
      occupation: data.occupation ? data.occupation : motherData.occupation,
      workLocation: data.workLocation
        ? data.workLocation
        : motherData.workLocation,
      motherImage: image ? image : motherData.motherImage,
    };

    try {
      setIsload(true);
      const submittedData = await PatchData(
        `mother/update-single-mother-by-patch/${motherData._id}`,
        newData
      );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          <label htmlFor="motherNameBn" className="block font-medium mb-1">
            মাতার নাম (বাংলা)
          </label>
          <input
            type="text"
